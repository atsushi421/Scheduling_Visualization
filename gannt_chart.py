...
def option_parser() -> str:
    ...
    arg_parser.add_argument('-y', '--y_axis',
                            required=True,
    ...
    return args.source_file_path, args.dest_dir, args.y_axis, args.highlight_deadline_miss, args.draw_legend
...
def main(source_file_path, dest_dir, y_axis, highlight_deadline_miss, draw_legend):
    ### json -> df
    ...
    source_df = pd.DataFrame(columns=['coreID',
                                      ...
                                      'Preemption',
                                      'Color'])
    color_dict = get_color_dict(source_dict, highlight_deadline_miss, y_axis)
    ...
    if(y_axis == 'core'):
        ...
        for i, task in enumerate(source_dict['taskSet']):
            ...
            if(highlight_deadline_miss and task['deadlineMiss']):
                ...
                color = color_dict[str(task['coreID'])]
        ...
        p.yaxis.major_label_text_font_size = '20pt'  # HACK
        p.xaxis[0].formatter = NumeralTickFormatter(format='0,0')
        hover = HoverTool(tooltips="Core: @coreID<br> \
                                    Job: @jobID<br>   \
                                    Start: @Start<br> \
                                    Finish: @Finish")
        p.add_tools(hover)
            ...
            for _, task_df in source_df.groupby(level=0):
                for _, task_series in task_df.droplevel(0).reset_index().iterrows():
                    ...
                    p.quad(left='Start',
                        ...
                        line_color='black',
                        hatch_color='black',
                        hatch_pattern=pattern[int(task_dict['coreID'][0]) % len(pattern)],
...
if __name__ == '__main__':
    source_file_path, dest_dir, y_axis, highlight_deadline_miss, draw_legend = option_parser()
    main(source_file_path, dest_dir, y_axis, highlight_deadline_miss, draw_legend)
